...
static void fs_init(void)
{
    /* SD card init & FAT mount are handled by Storage_Task (FreeRTOS).
     * Here we only check if already mounted; avoid blocking init
     * because HAL_SD_Init() hangs when no SD card is inserted.
     * Storage_Task will print mount status via serial after boot.
     */
}
